      ...
      {isLoading ? <PageSpinner /> : entries.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-xl border border-gray-100 bg-white">
          <table className="w-full table-fixed text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {entries.map((e, idx) => (
                <tr key={`${e.lineId ?? e.entryId ?? idx}`} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-gray-600 truncate">{e.sourceRef ?? e.entryNumber}</td>
